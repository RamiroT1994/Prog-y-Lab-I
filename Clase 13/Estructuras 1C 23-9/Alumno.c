...
void mostrarListadoAlumnos(eAlumno listadoDeAlumnos[], int tamA, eLocalidad listaLocalidades[], int tamL)
{
    int i,j;
    ...
    for(i=0; i<tamA; i++)
    {
        if(listadoDeAlumnos[i].estado==OCUPADO)
        {
            for(j=0; j<tamL ; j++)
            {
               mostrarUnAlumno(listadoDeAlumnos[i],listaLocalidades[j]);
            }
        }
    }
}
...
{
    printf("%4d %25s %8.2f",miAlumno.legajo,miAlumno.nombre, miAlumno.promedio);

    if(miAlumno.idLocalidad == miLocalidad.idLocalidad)
    {
        printf("%10s",miLocalidad.localidad);
    }
}
